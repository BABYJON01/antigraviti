        ...
        xrayUpload.addEventListener('change', async function(e) {
            ...
            try {
                ...
                if (prediction === -1 || data.valid === false) {
                    const errorMsg = data.detail ? data.detail : d.errorInvalid;
                    resultDiv.innerHTML = `<span style="color: var(--danger);"><i class="fa-solid fa-triangle-exclamation"></i> <strong>${errorMsg}</strong></span>`;
                    ...
                    document.querySelector('.diagnosis-card .severity').textContent = `Grade: ${prediction}${confPct}`;
                    ...
                    if(fuzzyElem) {
                        if (data.ai_report) {
                            // Format markdown-like text from Gemini (basic formatting)
                            const formattedReport = data.ai_report
                                .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
                                .replace(/\n/g, '<br>');
                            fuzzyElem.innerHTML = `<div class="ai-report-content">${formattedReport}</div>`;
                        } else {
